...
LOG_FILE = 'batch_operations_log.txt' # Updated log file name
...
def load_settings():
    ...
    # Path settings
    config['GLOBAL']['path_to_dosbox_exe'] = config['GLOBAL'].get('path_to_dosbox_exe', 'F:\\emulators\\dosbox_staging\\dosbox.exe')
    config['GLOBAL']['path_to_DBGL_root'] = config['GLOBAL'].get('path_to_DBGL_root', 'F:\\DBGL\\')
    config['GLOBAL']['path_to_temp_dir'] = config['GLOBAL'].get('path_to_temp_dir', 'C:\\Temp\\DBTemp\\')
    config['GLOBAL']['path_to_zip_dest_dir'] = config['GLOBAL'].get('path_to_zip_dest_dir', 'C:\\Temp\\')

    # Switch settings (Using ENABLED/DISABLED for consistency)
    config['GLOBAL']['preserve_mount_c'] = config['GLOBAL'].get('preserve_mount_c', 'ENABLED')
    config['GLOBAL']['automatic_mode_detection'] = config['GLOBAL'].get('automatic_mode_detection', 'DISABLED')
...
def create_zip_archive(settings, path='.', with_dosbox=False):
    ...
    zip_structure = settings.get('zip_dosbox_structure', 'FLAT')
...
def main_menu():
    """Main program loop with dynamic mode detection."""
    while True:
        settings = load_settings()
        
        mode = 'UNCERTAIN'
        if settings.get('automatic_mode_detection') == 'ENABLED':
            mode = detect_working_mode()

        os.system('cls' if os.name == 'nt' else 'clear')
        
        if mode == 'BATCH':
            batch_operations_menu(settings, came_from_main_menu=False)
            continue 

        # Program name in the header
        print(f"\n# {PROGRAM_NAME} ({VERSION}) - MODE: {mode} #")
        print("=======================================================")
        
        if mode == 'UNCERTAIN':
             print("\n| Mode: Uncertain |")
             print("This folder does not contain game configuration files (SINGLE).")
             print("If you are working with a group of games (BATCH), select option 9.")
             print("----------------------------------------------------------------")
        else:
             print("\n| Mode: Working with current Game Folder (SINGLE) |")
             print("----------------------------------------------------------------")


        print("| Configuration |")
        print("1 – Create Base Folders (cd, docs, drives/c)")
        print("2 – Create/Update !start.bat only (Dynamic relative link for USB)")
        print("3 – Create/Update dosbox.conf only (Profile + Modification)")
        print("4 – Create/Update !start.bat & dosbox.conf (Complete Setup)")
        print("---")
        ...
        print("| Archiving |")
        print("7 – Create PORTABLE ZIP Archive (With DOSBox)")
        print("8 – Create CLEAN ZIP Archive (Without DOSBox, for .conf association launch)")
        print("---")

        if mode == 'UNCERTAIN':
            print("| System Functions |")
            print("9 – BATCH OPERATIONS for ALL SUBFOLDERS >>")
            print("A – SETTINGS: Emulator Path / Profiles / Modes / ZIP Structure")
            print("X – CLEAN UP AND EXIT TOOL")
            print("Q – Exit Program")
            choice = input("\nEnter number/letter (1-9, A, X, Q): ").upper()
        else:
            print("| System Functions |")
            print("A – SETTINGS: Emulator Path / Profiles / Modes / ZIP Structure")
            print("X – CLEAN UP AND EXIT TOOL")
            print("Q – Exit Program")
            choice = input("\nEnter number/letter (1-8, A, X, Q): ").upper()
        ...
        if choice == 'Q':
            ...
            try: create_base_folders(settings)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '2':
            try: create_start_bat(settings)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '3':
            try: create_dosbox_conf(settings)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '4':
            try: run_complete_setup(settings)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '5':
            try: delete_backups(settings)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '6':
            try: delete_config_files(settings)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '7':
            try: create_zip_archive(settings, with_dosbox=True)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '8':
            try: create_zip_archive(settings, with_dosbox=False)
            except Exception as e: print(f"| ❌ ERROR: {e}")
            input("Press Enter to return...")
        elif choice == '9' and mode == 'UNCERTAIN':
            # Entry via choice 9: Q in BATCH menu should return here
